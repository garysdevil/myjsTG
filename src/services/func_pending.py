...
import myjsTG.services.tgfuncs as tgfuncs
from utils import gdata

# 全局参数
API_ID = gconfig.account_api_id  # Telegram API ID
API_HASH = gconfig.account_api_hash  # Telegram API Hash
PASSWORD = gconfig.account_password  # 两步验证密码（未设置可为空）
OLD_PASSWORD = gconfig.account_oldpassword  # 两步验证密码（未设置可为空）
...
async def process_account(client: TelegramClient, phone: str):
    ...
    await tgfuncs.list_authorizations(client)


async def change_password(client: TelegramClient, new_password: str, old_password: str, phone: str):
    await tgfuncs.change_password(client, new_password, old_password, phone)  # 更改2fa密码


async def remove_authorization(client: TelegramClient):
    table = await tgfuncs.list_authorizations(client, print_table=False)  # 获取所有授权设备
    for row in table:
        device_model = row[2]
        auth_id = row[1]
        # 根据设备类型进行踢掉设备
        if "Desktop" in device_model or "Samsung Galaxy" in device_model or "Asus ROG Phone" in device_model:
            ...
            await tgfuncs.kick_authorization(client, auth_id)  # 调用提取的函数
