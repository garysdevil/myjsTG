...
# 主函数
async def main(file_path='local/data.txt', start_line=1, end_line=None):

    # 获取提取的数据
    extracted_data = json.loads(get_extracted_data(file_path))
...
# 登录 Telegram 客户端
async def login_to_telegram(client, phone, code, password, proxy=None):
    """
    ...
    if not await client.is_user_authorized():
        ...
        # 获取验证码并尝试登录
        print("尝试从 URL 获取验证码...")
        code = get_code_from_url(code, proxy)
        if code:
            # 使用验证码进行登录
...
if __name__ == "__main__":
    # 从命令行传递 `start_line`, `end_line`, `file_path` 参数
    import argparse
    parser = argparse.ArgumentParser(description="Telegram 自动登录程序")
    parser.add_argument('--file-path', type=str, default='local/data.txt', help="指定数据文件路径，默认为 'local.data'")
    ...
    asyncio.run(main(file_path=args.file_path, start_line=args.start_line, end_line=args.end_line))
